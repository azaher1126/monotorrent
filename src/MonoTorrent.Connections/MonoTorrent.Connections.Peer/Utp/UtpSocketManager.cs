...
namespace MonoTorrent.Connections.Peer.Utp
{
    ...
    public sealed class UtpSocketManager : IDisposable
    {
        ...
        // Active uTP connections. For simplicity we key primarily by the receive connection id
        // (as BEP 29 / libtorrent do). For extra safety against ID collisions we can also consider
        // remote endpoint in lookup in the future.
        readonly Dictionary<ushort, UtpConnection> _connectionsByRecvId = new Dictionary<ushort, UtpConnection> ();
        ...
        /// Can be updated at runtime (affects new samples).
        /// </summary>
        ...
        public int ActiveConnections
        {
            ...
            {
                lock (_lock)
                    return _connectionsByRecvId.Count;
            }
        }
        ...
        public UtpSocketManager (int targetDelayMilliseconds = 75)
        {
            ...
            _tickTimer = new Timer (OnTick, null, TimeSpan.FromMilliseconds (100), TimeSpan.FromMilliseconds (100));
        }
        ...
        public UtpConnection CreateOutgoingConnection (IPEndPoint remoteEndPoint)
        {
            ...
            // Per BEP29 / libtorrent: for outgoing, our recv_id is random; the packet we send uses conn_id = recv_id (they will use it as their send_id)
            // Their reply will use conn_id = recv_id + 1 for us? See libtorrent utp_stream.cpp send_syn.
            ushort sendId = (ushort) ((recvId + 1) & 0xFFFF);

            var conn = new UtpConnection (this, remoteEndPoint, recvId, sendId, isIncoming: false, TargetDelayMilliseconds);

            lock (_lock) {
                // Guard against extremely unlikely id collision for a live connection.
                while (_connectionsByRecvId.ContainsKey (recvId))
                    recvId = (ushort) _random.Next (1, ushort.MaxValue);

                conn.RecvId = recvId; // in case we reassigned
                _connectionsByRecvId[recvId] = conn;
            }

            conn.SendSyn ();
            // Log.InfoFormatted ("Created outgoing uTP connection to {0}, recvId={1}", remoteEndPoint, recvId);
            return conn;
        }
        ...
        {
            lock (_lock) {
                if (!_connectionsByRecvId.TryAdd (conn.RecvId, conn)) {
                    // Collision — very rare. Reset and let the connection die.
                    // Log.Error ($"uTP recvId collision for {conn.RecvId} — dropping new connection");
                    conn.Abort ();
                    return;
                }
            }
        ...
        {
            lock (_lock) {
                _connectionsByRecvId.Remove (conn.RecvId);
            }
        }
        ...
        void OnDatagramReceived (ReadOnlyMemory<byte> buffer, CompactEndPoint remote)
        {
            ...
            UtpConnection? conn;
            lock (_lock) {
                _connectionsByRecvId.TryGetValue (header.ConnectionId, out conn);
            }
            ...
            if (conn == null) {
                ...
                conn = new UtpConnection (this, remoteEp, ourRecvId, ourSendId, isIncoming: true, TargetDelayMilliseconds);

                lock (_lock) {
                    // If our chosen recvId is already used, drop (extremely unlikely during the lifetime of a conn).
                    if (_connectionsByRecvId.ContainsKey (ourRecvId)) {
                        // (uTP incoming recvId collision {ourRecvId})
                        return;
                    }
                    _connectionsByRecvId[ourRecvId] = conn;
                }
        ...
        void OnTick (object? state)
        {
            ...
            lock (_lock) {
                snapshot = new List<UtpConnection> (_connectionsByRecvId.Values);
            }
        ...
        public void Dispose ()
        {
            ...
            lock (_lock) {
                ...
                foreach (var c in _connectionsByRecvId.Values)
                    c.Abort ();

                _connectionsByRecvId.Clear ();
            }
        }
