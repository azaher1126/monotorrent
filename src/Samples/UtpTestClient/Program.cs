...
using MonoTorrent.Connections.Peer;   // For UtpPeerConnection type check
...
namespace UtpTestClient
{
    ...
    class Program
    {
        ...
        static async Task RunAsync (string input, int listenPort, int utpTargetDelayMs, bool enableLogging, CancellationToken token)
        {
            ...
            if (enableLogging) {
                // In DEBUG this is often already on, but this forces it for Release too if the factory is registered.
                // The library will log to any registered listeners (we just write to Console here for simplicity).
                MonoTorrent.Logging.LoggerFactory.Register (new MonoTorrent.Logging.TextWriterLogger (Console.Out));
            }
            ...
            // outgoing transport when UtpEnabled=true).
            ...
            Console.WriteLine ();
            Console.WriteLine ($"Engine listening. UtpEnabled={engine.Settings.UtpEnabled}  UtpTargetDelay={engine.Settings.UtpTargetDelayMilliseconds}ms");
            if (listenPort > 0)
